...
(function initializeForecastBuilderPreview(window) {
  ...
  const state = {
    ...
    loading: false, requestSerial: 0, jobsByEngineer: new Map(), expanded: new Set(), contextExpanded: new Set(), showAllHistory: new Set(), charts: new Map(), drafts: new Map(),
    workDoneUploadedByYear: {}, jobComments: {},
    historyLoadedForYear: '',
  ...
  function renderProfile(jobNumber) {
    const showAll = state.showAllHistory.has(draftKey(jobNumber));
    return `<section class="preview-profile" aria-label="Standard Job period profile"><div class="preview-profile-header"><div><h4>Standard Job profile</h4><p>Current V0 includes unsaved changes. Historical lines blend corrected Work Done with the final effective forecast tail.</p></div><button type="button" class="group-action-button" data-show-history="${escapeHtml(jobNumber)}" aria-pressed="${showAll}">${showAll ? 'Show previous FY only' : 'Show all history'}</button></div><div class="preview-chart-wrap"><canvas data-profile-chart="${escapeHtml(jobNumber)}" aria-label="P1 to P13 Standard Job profile" role="img"></canvas></div><div class="preview-profile-transition" data-profile-transition="${escapeHtml(jobNumber)}"></div></section>`;
  }
  function profileSeries(jobNumber) {
    const draft = getDraft(jobNumber);
    const current = Object.fromEntries(PERIODS.map(period => [period, Object.values(draft.rows).reduce((sum, row) => sum + (Number(row.periods[period]) || 0), 0)]));
    const selectedNumber = Number(String(state.selectedYear).replace(/^FY/i, ''));
    ...
    return { current, history: years.map(year => {
      const rows = Object.keys(draft.rows).map(workGroup => window.getPlanningContext({ selectedYear: state.selectedYear, historyYears: [year], jobNumber: normalizeJob(jobNumber), workGroup,
        effectiveForecastsByYear: state.effectiveForecastsByYear, workDoneByYear: state.workDoneByYear,
        workDoneUploadedByYear: state.workDoneUploadedByYear, resolveWorkGroupCode: window.resolveWorkGroupCode })[0]);
      const coverage = rows[0]?.coverage || { lastPeriod: 0, label: 'not uploaded' };
      const periods = Object.fromEntries(PERIODS.map(period => [period, rows.reduce((sum, context) => sum + (Number((coverage.label === 'not uploaded' ? context.forecastPeriods : window.copyPlanningProfile(context, 'work-done'))[period]) || 0), 0)]));
  ...
  function renderProfileCharts() {
    ...
    document.querySelectorAll('[data-profile-chart]').forEach(canvas => {
      ...
      const datasets = [{ label: `${state.selectedYear} V0 (current draft)`, data: PERIODS.map(period => series.current[period]), borderColor: '#2563eb', backgroundColor: '#2563eb', borderWidth: 3, tension: .2 }];
